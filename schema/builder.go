...
func NewBuilder(name string, conns ...string) *Builder {
	...
	return &Builder{
		name:                name,
		conn:                conn,
		prefix:              util.ConfigHelper.GetString(fmt.Sprintf("db.%s.prefix", conn), ""),
		charset:             util.ConfigHelper.GetString(fmt.Sprintf("db.%s.charset", conn), "utf8"),
		collation:           util.ConfigHelper.GetString(fmt.Sprintf("db.%s.collation", conn), "utf8_unicode_ci"),
...
func (t *Builder) Run() {
	...
	for _, sql := range sqls {
		if _, err := util.DbHelper.Exec(sql, t.conn); err != nil {
			util.SysHelper.Exit("SQL Execute Fail, sql:%s, err: %s", sql, err)
		}
	}
}
...
func (t *Builder) SoftDeletes(column string, precision int) *ColumnDefinition {
	return t.Timestamp(column).Nullable(true)
}
...
func (t *Builder) String(name string, length ...int) *ColumnDefinition {
	...
	if _length <= 0 {
		_length = t.defaultStringLength
	}

	return t.addColumn(&ColumnDefinition{
		ColumnName: name,
		ColumnType: fmt.Sprintf("VARCHAR(%d)", _length),
	})
}
...
func (t *Builder) UnsignedBigInteger(name string, autoIncrement ...bool) *ColumnDefinition {
	...
	if len(autoIncrement) > 0 {
		_autoIncrement = autoIncrement[0]
	}

	return t.addColumn(&ColumnDefinition{
		ColumnName:          name,
		ColumnType:          "bigint",
		ColumnAutoIncrement: _autoIncrement,
		ColumnUnsigned:      true,
	})
}
...
func (t *Builder) DropTableIfExists() *Command {
	return t.addCommand("dropTableIfExists")
}
...
func (t *Builder) compileDropTable(c *Command) string {
	return fmt.Sprintf("DROP TABLE %s", t.wrapTable())
}

func (t *Builder) compileDropTableIfExists(c *Command) string {
